 ...
 *
 * The "lambda expression" assigned to an object of "ObjDoubleConsumer" type is used to define its "accept()" which
 * eventually applies the given operation on its argument.
 *
 * It takes in a "double-valued" and a "T-valued" argument and is expected to operate without any side effects.
 * It is more like using an object of type "BiConsumer<T, Double>" except the fact that one is a reference and one is
 * a primitive java_regular.data type in this case.
 *
 * Hence we will be obtaining one reference and one value when this function is called.
 */
public class ObjDoubleConsumerMain {
